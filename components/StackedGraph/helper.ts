...
export const modifyData = (keyCounts: { [key: string]: number }): KeyData[] => {
  ...
  const sum = keys.reduce(
    (accumulator: number, key: string) => accumulator + keyCounts[key],
    0
  );
...
const combineKeyData = (keys: KeyData[]): KeyData => {
  ...
  const sumPercent = keys.reduce(
    (accumulator: number, key: KeyData) => accumulator + key.percentage,
    0
  );
  const sumCount = keys.reduce(
    (accumulator: number, key: KeyData) => accumulator + key.count,
    0
  );
  return {
